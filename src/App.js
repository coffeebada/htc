...
const BOX_STYLE = {
  ...
  holeNumber: { width: '90px', height: '60px', border: '2px solid #475569', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '900', fontSize: '26px', color: '#cbd5e1', backgroundColor: '#1e293b', margin: '8px 0', userSelect: 'none', lineHeight: '60px' }
};
...
const MODAL_STYLE = {
  ...
  modalContent: { backgroundColor: '#111827', width: '520px', borderRadius: '24px', padding: '35px', border: '1px solid #374151', color: 'white' },
...
// 🎯 [도면 100% 완전 동기화 확정 레이아웃 데이터셋]: 7~10번 홀 상단 하늘색 벤딩 및 하단 주황색 오버드로우 스택 전수 매핑 교정 완료
const HARP_LAYOUT = {
  ...
  bottomSpecials: [
    ...
    [85],                   // 7번 홀 bottom: Db6 (🎯오버드로우 주황색 매칭 완료)
    [89],                   // 8번 홀 bottom: F6 (🎯오버드로우 주황색 매칭 완료)
    [92],                   // 9번 홀 bottom: Ab6 (🎯오버드로우 주황색 매칭 완료)
    [97]                    // 10번 홀 bottom: Db7 (🎯오버드로우 주황색 매칭 완료)
  ]
};
...
function HarmonicaRoom({ baseFreq, setBaseFreq, tolerance, setTolerance, programSettingMode, setProgramSettingMode, reverbMix, setReverbMix, onRouteClick }) {
  ...
  useEffect(() => {
    if (micGainNode.current) micGainNode.current.gain.value = micVolume * 1.0; 
    if (micRecordBooster.current) micRecordBooster.current.gain.value = micVolume * 1.2; 
  }, [micVolume]);
  ...
  useEffect(() => {
    ...
    micGainNode.current.gain.value = micVolume * 1.0; 
    micGainNode.current.connect(mixedBus.current);
    
    micRecordBooster.current = ctx.createGain();
    micRecordBooster.current.gain.value = micVolume * 1.2; 
  ...
  const startMic = async () => {
    try {
      ...
      compressor.release.setValueAtTime(0.06, ctx.currentTime);
  ...
  return (
    <div style={BOX_STYLE.contentWrapper}>
      ...
      <div style={BOX_STYLE.gridContainer}>
        {HARP_LAYOUT.holes.map((h, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
            ...
            {/* 세로 정중앙 정렬 배치가 처리된 중앙 홀 가이드 넘버 사각형 */}
            ...
            {/* 카피라이트 텍스트 크기를 REC MIX 버튼의 크기인 14px로 동일 매칭 */}
            {h === 10 && (
              <div style={{ position: 'absolute', bottom: '-5px', right: '0px', width: '650px', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', pointerEvents: 'none', zIndex: 10, fontFamily: 'sans-serif', lineHeight: '1.4' }}>
                ...
                <div style={{ color: '#ffffff', fontSize: '14px', fontWeight: '600' }}>Copyright ⓒ 2026 CoffeeBada Lee, Choong-Koo All Rights Reserved.</div>
                <div style={{ color: '#ffffff', fontSize: '14px', fontWeight: '600', marginTop: '2px' }}>Contact : 279.lee@gmail.com</div>
              </div>
            )}
          </div>
        ))}
      ...
      {/* 모달 설정창 내부 글자 크기를 극대화하여 시인성을 확보한 세팅 팝업창 */}
      {showSettings && (
        <div style={MODAL_STYLE.modalOverlay} onClick={() => setShowSettings(false)}>
          <div style={MODAL_STYLE.modalContent} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h2 style={{ fontSize: '28px', margin: 0, color: '#10b981', fontWeight: '900' }}>Settings</h2>
              <X size={32} style={{ cursor: 'pointer', color: '#94a3b8' }} onClick={() => setShowSettings(false)} />
            </div>

            <div style={{ marginBottom: '20px', padding: '18px', backgroundColor: '#1f2937', borderRadius: '12px', border: '1px solid #374151' }}>
              <span style={{ fontSize: '18px', color: '#cbd5e1', fontWeight: 'black', display: 'block', marginBottom: '12px' }}>Program Key Mode</span>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <button 
                  onClick={() => setProgramSettingMode('standard')} 
                  style={{ padding: '14px', backgroundColor: programSettingMode === 'standard' ? '#10b981' : '#374151', color: programSettingMode === 'standard' ? 'black' : 'white', border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: '900', cursor: 'pointer' }}
                >
                  기본 세팅 (Standard)
                </button>
                <button 
                  onClick={() => setProgramSettingMode('low')} 
                  style={{ padding: '14px', backgroundColor: programSettingMode === 'low' ? '#10b981' : '#374151', color: programSettingMode === 'low' ? 'black' : 'white', border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: '900', cursor: 'pointer' }}
                >
                  로우 세팅 (Low Key)
                </button>
              </div>
            ...
            <div style={{ marginBottom: '20px', padding: '18px', backgroundColor: '#1f2937', borderRadius: '12px', border: '1px solid #374151' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span style={{ fontSize: '18px', color: '#cbd5e1', fontWeight: 'black' }}>Reference Pitch</span>
                <span style={{ fontSize: '18px', color: '#10b981', fontWeight: '900' }}>{baseFreq} Hz</span>
              </div>
            ...
            <div style={{ marginBottom: '20px', padding: '18px', backgroundColor: '#1f2937', borderRadius: '12px', border: '1px solid #374151' }}>
              <span style={{ fontSize: '18px', color: '#cbd5e1', fontWeight: 'black', display: 'block', marginBottom: '12px' }}>Tuner Tolerance (Cents)</span>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '8px' }}>
                {[5, 10, 15, 20].map((val) => (
                  <button key={val} onClick={() => setTolerance(val)} style={{ padding: '12px 10px', backgroundColor: tolerance === val ? '#10b981' : '#374151', color: tolerance === val ? 'black' : 'white', border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: '900', cursor: 'pointer' }}>
                    ±{val}¢
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: '20px', padding: '18px', backgroundColor: '#1f2937', borderRadius: '12px', border: '1px solid #374151' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                <span style={{ fontSize: '18px', color: '#cbd5e1', fontWeight: 'black' }}>Mic Reverb Effect</span>
                <button onClick={() => setUseReverb(!useReverb)} style={{ padding: '8px 18px', backgroundColor: useReverb ? '#10b981' : '#ef4444', border: 'none', color: useReverb ? 'black' : 'white', borderRadius: '8px', fontSize: '14px', fontWeight: '900', cursor: 'pointer' }}>{useReverb ? 'ON' : 'OFF'}</button>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '13px', color: '#94a3b8', fontWeight: 'bold', width: '90px' }}>REVERB MIX</span>
                <input type="range" min="0" max="1" step="0.01" disabled={!useReverb} value={reverbMix} onChange={(e) => setReverbMix(parseFloat(e.target.value))} style={{ flex: 1, cursor: useReverb ? 'pointer' : 'not-allowed' }} />
                <span style={{ fontSize: '14px', color: '#10b981', fontWeight: 'bold', width: '40px', textAlign: 'right' }}>{Math.round(reverbMix * 100)}%</span>
              </div>
            </div>
            <button onClick={() => setShowSettings(false)} style={{ ...MODAL_STYLE.saveBtn, fontSize: '20px' }}>SAVE & CLOSE</button>
          </div>
        </div>
      )}
    </div>
...
// 🎯 [사각형 개별 노드 모듈 컴포넌트 - 기본음 사각형 컬러 및 텍스트 폰트 반전 최적화 완료]
function NoteBox({ semi, getNote, getAbsSemi, activeAbsoluteSemi, cents, limit, onStart, onStop, isTopBb, boxKey, clickedCellKey, isBlowZone, isDrawZone, holeNum, isBasicBlow, isBasicDraw }) {
  ...
  // 🎨 [도면 색상 가이드라인 100% 완전 동기화 및 기본음 사각형 컬러 교정 필터]
  let bgColor = '#1e293b'; 
  let borderStyle = '1px solid #334155';
  
  if (isActive) {
    if (isCurrentlyClicked) {
      bgColor = '#22c55e'; // 연주 클릭 시 시각 피드백 (녹색)
    } else {
    ...
    if (isTopBb) {
      ...
      // 🎯 [요구사항 반영]: 기본 불고 마시는 숨 사각형 컬러를 진한 블루그레이(#475569)로 정밀 교정
      bgColor = '#475569'; 
    } else if (isDrawZone) {
  ...
  // 🎯 [요구사항 반영]: 배경색 변경에 맞춰 시인성을 극대화하기 위해 기본음 폰트 글자 색상을 흰색(#ffffff)으로 연동 반전
  const fontColor = (isBasicBlow || isBasicDraw) ? '#ffffff' : 'black';
